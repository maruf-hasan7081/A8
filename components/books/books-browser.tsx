...
const categories: Array<"All" | BookCategory> = ["All", "Story", "Tech", "Science"];

interface BooksBrowserProps {
  initialCategory?: string;
}

function normalizeCategory(category?: string): "All" | BookCategory {
  return categories.includes(category as "All" | BookCategory)
    ? (category as "All" | BookCategory)
    : "All";
}

export function BooksBrowser({ initialCategory }: BooksBrowserProps) {
  const [books, setBooks] = useState<Book[]>([]);
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<"All" | BookCategory>(
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[250px_1fr]">
              ...
              >
                ...
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${
                    category === item ? "bg-white/15" : "bg-white"
                  }`}
                >
                  {count}
                </span>
      ...
      <section>
        ...
        {loading ? (
          <div className="grid min-h-80 place-items-center rounded-3xl border border-dashed border-[#173f35]/20 bg-white/50">
            <LoadingSpinner label="Loading books" />
          </div>
        ) : error ? (
          <div className="alert alert-error mt-8 rounded-2xl" role="alert">
            <span>{error}</span>
          </div>
        ) : filteredBooks.length === 0 ? (
          <div className="mt-8 grid min-h-80 place-items-center rounded-3xl border border-dashed border-[#173f35]/20 bg-white/60 p-8 text-center">
            <div>
