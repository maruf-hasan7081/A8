...
export function BookCard({ book }) {
    return (<article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-[#173f35]/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative aspect-[4/3] overflow-hidden bg-[#ece4d3] p-6">
        <Image src={book.image_url} alt={`Cover of ${book.title}`} width={360} height={520} className="book-shadow mx-auto h-full w-auto rounded-md object-cover transition duration-500 group-hover:scale-105 group-hover:-rotate-1"/>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <Link href={`/books/${book.id}`} className="btn mt-5 w-full border-[#173f35]/15 bg-[#173f35] text-white hover:bg-[#24594b]">
          View Details
