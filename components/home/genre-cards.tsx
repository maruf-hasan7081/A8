...
export function GenreCards() {
  return (
    <section className="border-y border-[#173f35]/10 bg-white/55 py-20">
      <div className="page-shell">
        <SectionHeading
          eyebrow="Browse by mood"
          title="Three shelves, endless directions"
          description="Choose a category and quickly narrow the collection to the kind of book you need today."
          centered
        />
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {genres.map(({ title, description, icon: Icon, className }) => (
            <Link
              key={title}
              href={`/books?category=${title}`}
              className={`${className} group rounded-[2rem] border border-[#173f35]/10 p-7 transition hover:-translate-y-1 hover:shadow-xl`}
            >
